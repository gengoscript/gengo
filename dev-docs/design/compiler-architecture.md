...
- **Generic functions** (`func name[T constraint](...)`) support an optional structural constraint (`numeric`/`ordered`/`comparable`) checked against the concrete argument's primitive or named-type base, and explicit-type-argument call syntax (`name[T, U](args)`). Type arguments are also inferred from ordinary call arguments (`identity(42)` infers `T = int`, no `[int]` needed) and from generic return positions — this is a separate mechanism from struct/variant instantiation inferring a type argument from a literal's shape (`Stack[int]{ items: [] }`), but both exist. What inference does **not** do is enforce a declared constraint: `checkTypeArgConstraints` only runs for an explicit `name[T, U](args)` call; an inferred call that resolves to a type violating the constraint currently compiles anyway (tracked as a known limitation, `tests/spec/322_generic_constraint_inference_gap.gengo`). Write explicit type arguments when constraint enforcement matters.
...
- **Operator overloading (#210) is name-disambiguated, not keyword-disambiguated.** Eight reserved method names (`__add__`, `__sub__`, `__mul__`, `__div__`, `__rem__`, `__neg__`, `__eq__`, `__compare__`) are ordinary methods — no `operator` modifier keyword, unlike Kotlin. This works because Gengo has no separate impl-block boundary (methods are always declared alongside their receiver type, unlike Rust's `impl Trait for Type` elsewhere), so there's no coherence problem a keyword would need to guard against; the reserved names alone are distinctive enough that nobody accidentally collides with them the way `add`/`eq` would. The compile-time desugar (`compiler_expr.zig`'s `tryEmitDunderBinaryOp`) reuses the existing static method-dispatch call shape (`get_global`+`swap`+`call`) but must run *before* parsing the RHS, not after — the callee has to be inserted while only the receiver is on the stack. Because the desugar is compile-time-only, it cannot fire inside a type-erased generic function body (no monomorphization), so `vm.zig` also carries a runtime fallback (`tryStructDunderBinary`/`tryStructDunderUnary`) using the same reentrant-`callFunction` idiom already used for named-type predicates — without it, a struct satisfying `ordered`/`comparable` via `__compare__`/`__eq__` would pass the generic constraint check at the call site and then panic the first time the function body actually used the operator, which would have been a "fails loudly" violation, not an acceptable gap.
