...
**Typed-arithmetic intrinsic selection.** `selectTypedArithmeticOp`/`selectTypedComparisonOp`/`selectTypedNotEqualOp` compare the `ExprPrimInfo` of both operands: if they share the same tracked `PrimType` (`.int` or `.float`) and **neither operand is a compile-time constant**, the generic op (`.add`, `.eq`, `.lt`, ...) is swapped for its typed sibling (`.add_int`, `.eq_int`, `.lt_int`, ...). The constant exclusion is deliberate — it keeps a `constant + constant` sequence visible to the chunk-level constant folder (below), which a typed op would hide. There's also **zero-comparison specialization**: `x == 0`/`x < 0` against a non-constant int binding rewrites to `.eqz_int`/`.ltz_int`/etc., popping the RHS's already-pushed `0` and emitting the fused unary compare instead of a full binary compare.
...
- **Contextual keywords fail loudly on misclassification.** `range`/`cycle`/`default`/`predicate`/`message` are recognized only by clause-position lookahead; a token that looks like one of these outside clause position is treated as a plain identifier, and any resulting mismatch is a compile error, never a silent reinterpretation.
