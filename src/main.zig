    ...
    if (emit_gbc_path) |out_path| {
        ...
        const gbc_bytes = gbc_writer.write(runtime.chunk_state, std.heap.page_allocator, .{ .root_source = src }) catch |err| {
            ...
            if (err == error.UnsupportedConstant) {
                io.werr(" (this script uses a feature GBC caching doesn't support yet: a predicate declared inside a function body (rather than at module/type scope), or a closure with real captures stored as a constant — see issue #5)");
            }
            io.werr("\n");
            die(1);
        };
