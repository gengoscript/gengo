...
pub fn stackEffect(op: Op, code: []const u8, ip: usize) struct { pop: u16, push: u16 } {
    return switch (op) {
        .constant, .null_val, .true_val, .false_val, .get_local, .get_upvalue, .get_global, .make_closure, .tuple_get_keep, .get_local_const_eq, .get_local_const_sub, .get_local_const_add, .get_local_const_lt, .get_local_const_gt, .get_global_const_eq, .get_global_const_sub, .get_global_const_add, .get_global_const_lt, .get_local_get_field, .task_self, .task_receive => .{ .pop = 0, .push = 1 },
