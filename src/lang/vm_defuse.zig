...
fn emitExpanded(
    ...
    switch (op) {
        // ── Fused return variants ────────────────────────────────────────────
        .get_local_ret => {
            dst[0] = opByte(.get_local); dst[1] = rb(code, old_ip, 1);
            dst[2] = opByte(.ret);
        },
        .add_ret => {
            dst[0] = opByte(.add);
        ...
        .const_eq => {
            ...
            dst[1] = rb(code, old_ip, 1); dst[2] = rb(code, old_ip, 2);
            dst[3] = opByte(.eq);
        },
        .const_sub => {
            dst[0] = opByte(.constant);
        ...
        .get_local_const_eq => {
            dst[0] = opByte(.get_local); dst[1] = rb(code, old_ip, 1);
            dst[2] = opByte(.constant); dst[3] = rb(code, old_ip, 3); dst[4] = rb(code, old_ip, 4);
            dst[5] = opByte(.eq);
        },
        .get_local_const_sub => {
            dst[0] = opByte(.get_local); dst[1] = rb(code, old_ip, 1);
        ...
        .get_local_const_sub_call => {
            ...
            dst[2] = opByte(.constant); dst[3] = rb(code, old_ip, 3); dst[4] = rb(code, old_ip, 4);
            dst[5] = opByte(.sub);
            dst[6] = opByte(.call); dst[7] = rb(code, old_ip, 5); dst[8] = 0xFF; dst[9] = 0xFF;
        },
        .get_local_const_sub_call_tail => {
            dst[0] = opByte(.get_local); dst[1] = rb(code, old_ip, 1);
            dst[2] = opByte(.constant); dst[3] = rb(code, old_ip, 3); dst[4] = rb(code, old_ip, 4);
            dst[5] = opByte(.sub);
            dst[6] = opByte(.call_tail); dst[7] = rb(code, old_ip, 5); dst[8] = 0xFF; dst[9] = 0xFF;
        },
        ...
        .get_local_const_eq_jif_pop => {
            ...
            dst[0] = opByte(.get_local); dst[1] = rb(code, old_ip, 1);
            dst[2] = opByte(.const_eq); dst[3] = rb(code, old_ip, 3); dst[4] = rb(code, old_ip, 4);
        ...
        .get_local_const_lt_jif_pop => {
            const tgt = instr.jump_target.?;
            dst[0] = opByte(.get_local); dst[1] = rb(code, old_ip, 1);
            dst[2] = opByte(.const_lt); dst[3] = rb(code, old_ip, 3); dst[4] = rb(code, old_ip, 4);
            dst[5] = opByte(.jif_pop);
            pu32(dst, 6, fwdOff(ip_map, tgt, new_end));
        },
        .get_local_const_gt_jif_pop => {
            const tgt = instr.jump_target.?;
            dst[0] = opByte(.get_local); dst[1] = rb(code, old_ip, 1);
            dst[2] = opByte(.const_gt); dst[3] = rb(code, old_ip, 3); dst[4] = rb(code, old_ip, 4);
            dst[5] = opByte(.jif_pop);
            pu32(dst, 6, fwdOff(ip_map, tgt, new_end));
        },
        ...
        .get_global_const_lt_jif_pop => {
            const tgt = instr.jump_target.?;
            dst[0] = opByte(.get_global); dst[1] = rb(code, old_ip, 1); dst[2] = rb(code, old_ip, 2);
            dst[3] = 0xFF; dst[4] = 0xFF;
            dst[5] = opByte(.const_lt); dst[6] = rb(code, old_ip, 6); dst[7] = rb(code, old_ip, 7);
        ...
        .set_global_loop => {
            const tgt = instr.jump_target.?;
            dst[0] = opByte(.set_global); dst[1] = rb(code, old_ip, 1); dst[2] = rb(code, old_ip, 2);
            dst[3] = 0xFF; dst[4] = 0xFF;
            dst[5] = opByte(.loop);
        ...
        .close_upvalue_loop => {
            const tgt = instr.jump_target.?;
            dst[0] = opByte(.close_upvalue); dst[1] = rb(code, old_ip, 1);
            dst[2] = opByte(.loop);
        ...
        .get_local_get_field => {
            ...
            dst[3] = rb(code, old_ip, 3); dst[4] = rb(code, old_ip, 4); // name_hi, name_lo
        ...
        .get_global, .set_global => {
            ...
            dst[0] = rb(code, old_ip, 0); dst[1] = rb(code, old_ip, 1); dst[2] = rb(code, old_ip, 2);
            dst[3] = 0xFF; dst[4] = 0xFF;
        },
        .get_field, .set_field => {
